...
export default function Login() {
  ...
  const [isDesktop, setIsDesktop] = useState(false); // ⬅️ NEW STATE: To control password visibility
  ...
  const handleLogin = async (e) => {
    // Made async
    e.preventDefault();
    setError(""); // Clear previous errors // Call the real login function from AuthContext
  ...
  if (!isDesktop) {
    return (
      <div className="flex flex-col items-center justify-center h-screen text-center p-4 gap-6 bg-[#F2EFEE]">
        {" "}
        <div className="bg-white rounded-2xl shadow-lg px-8 py-10 flex flex-col items-center gap-6 border border-gray-100">
          ...
          <h1 className="text-xl font-semibold text-[#474545]">
            Sorry, the Team login page is only accessible on a laptop or desktop
            screen.{" "}
          </h1>{" "}
        </div>{" "}
      </div>
    ...
    <div className="flex items-center justify-center min-h-screen bg-[#F2EFEE] p-4">
      ...
      >
        ...
        <div className="relative">
          ...
          <input // ⬅️ CONDITIONAL TYPE
            type={showPassword ? "text" : "password"}
            value={password}
            onChange={(e) => setPassword(e.target.value)}
            placeholder="Password"
            required // ⬅️ ADD pr-10 for toggle button spacing
