...
const MiddleArea = ({ onProjectClick, filterState, onFilterChange }) => {
  ...
  const TableCell = ({ children, className = "" }) => (
    <td
      className={`px-3 py-2 border-b border-gray-200 ${COLOR_TEXT_BODY} text-sm ${className}`}
    >
      {children}
    </td>
  );
  ...
  const TableHeaderCell = ({ children, className = "" }) => (
    <th
      className={`px-3 py-2 font-medium ${COLOR_TEXT_BODY} opacity-70 ${className}`}
    >
      {children}
    </th>
  );
      ...
      <div className="flex flex-col md:flex-row justify-between items-center mb-6 gap-4">
        ...
        {!isAddingOrModifying && (
          <div className="relative w-full md:w-1/2">
            {" "}
            <FiSearch
              className={`absolute left-3 top-1/2 -translate-y-1/2 ${COLOR_TEXT_BODY} opacity-50`}
            />{" "}
            <input
              type="text"
            ...
            <div className="relative" ref={filterMenuRef}>
              ...
              {isFilterMenuOpen && (
                <div className="absolute top-full mt-2 right-0 md:left-0 w-64 bg-white border border-gray-200 rounded-lg shadow-lg p-4 z-20">
                  {" "}
                  <h3 className={`font-bold ${COLOR_TEXT_BODY} mb-3 text-sm`}>
                    Filter by Category
                  </h3>{" "}
                          ...
                          <div className="flex flex-col gap-1 mt-1 ml-4 border-l border-gray-200 pl-2">
                            ...
                            {filterOptions[category].map((sub) => (
                              <button
                                ...
                                className={`w-full text-left px-2 py-1 text-xs rounded-md transition ${
              ...
              {filteredProjects.map((project, index) => (
                ...
                >
                  ...
                  <TableCell>
                    ...
                    <img
                      src={
                        project.previewImageUrl || "https://picsum.dev/300/200"
                      }
                      alt={project.name}
