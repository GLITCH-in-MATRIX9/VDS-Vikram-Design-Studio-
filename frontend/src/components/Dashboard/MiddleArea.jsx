...
const MiddleArea = ({ onProjectClick, filterState, onFilterChange }) => {
  ...
  const filteredProjects = projects.filter((project) => {
    ...
    const matchesSubCategory = selectedSubCategory
      ? project.subCategory.toUpperCase() === selectedSubCategory.toUpperCase()
      : true;
  ...
  const handleCategoryClick = (category) =>
    onFilterChange({ selectedCategory: selectedCategory === category ? null : category, selectedSubCategory: null });
  ...
  const TableCell = ({ children, className = "" }) => (
    <td className={`px-3 py-2 border-b border-gray-200 text-sm ${className}`} style={{ color: COLOR_TEXT_BODY }}>
      {children}
    </td>
  );

  const TableHeaderCell = ({ children, className = "" }) => (
    <th className={`px-3 py-2 font-medium opacity-70 ${className}`} style={{ color: COLOR_TEXT_BODY }}>
      {children}
    </th>
  );
    ...
    <div className="flex-1 flex flex-col p-6">
      ...
      <div className="flex flex-col md:flex-row justify-between items-center mb-6 gap-4">
        {!isAddingOrModifying && (
          <div className="relative w-full md:w-1/2">
            ...
            <input
              ...
              style={{ borderColor: COLOR_BORDER, color: COLOR_TEXT_BODY }}
        ...
        <div className="flex gap-3">
          {!isAddingOrModifying && (
            <div className="relative" ref={filterMenuRef}>
              <button
                ...
                style={{ borderColor: COLOR_BORDER, color: COLOR_TEXT_BODY }}
              >
                <FiSettings /> Filter <FiChevronDown className={`${isFilterMenuOpen ? "rotate-180" : ""} transform transition-transform text-xs`} />
              </button>
              {isFilterMenuOpen && (
                <div className="absolute top-full mt-2 right-0 md:left-0 w-64 bg-white border rounded-lg shadow-lg p-4 z-20" style={{ borderColor: COLOR_BORDER }}>
                  <h3 className="font-bold mb-3 text-sm" style={{ color: COLOR_TEXT_BODY }}>Filter by Category</h3>
                  <div className="flex flex-col gap-1 mb-4">
                    {Object.keys(filterOptions).map((category) => (
                      <div key={category}>
                        <button
                          onClick={() => handleCategoryClick(category)}
                          className={`w-full text-left px-3 py-2 text-sm rounded-md transition flex items-center justify-between ${selectedCategory === category ? "bg-gray-700 text-white font-semibold" : "text-gray-700 hover:bg-gray-100"}`}
                        >
                          {category} <FiChevronDown className={`${selectedCategory === category ? "rotate-180" : ""} transform transition-transform text-xs`} />
                        </button>
                          ...
                          <div className="flex flex-col gap-1 mt-1 ml-4 border-l border-gray-200 pl-2">
                            {filterOptions[category].map((sub) => (
                              <button
                                ...
                                className={`w-full text-left px-2 py-1 text-xs rounded-md transition ${selectedSubCategory === sub ? "bg-gray-700 text-white" : "text-gray-700 hover:bg-gray-100"}`}
                              >
                                {sub}
                              </button>
                            ))}
          ...
          >
            {isAddingOrModifying ? <><FiArrowLeft /> Back</> : <><FiPlus /> Add Project</>}
          </button>
        </div>
      </div>

      {/* Project Table / AddProject Form */}
      <div className="flex-1 overflow-y-auto border-t max-h-[80vh]" style={{ borderColor: COLOR_BORDER }}>
        {editingProject || showAddProject ? (
          <AddProject projectToEdit={editingProject} onCancel={onCancelModify} refreshProjects={fetchProjects} />
        ) : isLoading ? (
          <ProjectTableSkeleton TableHeaderCell={TableHeaderCell} TableCell={TableCell} />
        ) : (
          <table className="w-full border-collapse text-left text-sm">
            <thead className="sticky top-0 z-10" style={{ backgroundColor: COLOR_HEADER_BG }}>
              <tr>
            ...
            <tbody>
              {filteredProjects.map((project, index) => (
                ...
                >
                  ...
                  <TableCell className="max-w-[150px] truncate font-medium">{project.name}</TableCell>
...
const ProjectTableSkeleton = ({ TableHeaderCell, TableCell }) => {
  const SkeletonRow = () => (
    <tr className="animate-pulse">
      ...
      <TableCell className="text-center"><div className="h-3 w-3 bg-gray-200 rounded-full mx-auto"></div></TableCell>
      <TableCell><div className="h-8 w-16 bg-gray-200 rounded"></div></TableCell>
