    ...
    <div className="flex-1 flex flex-col p-6 bg-white">
        ...
        {!isAddingOrModifying && (
          <div className="relative w-full md:w-1/2">
            ...
            <input
              ...
              className="border border-gray-300 px-10 py-2 w-full text-sm placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-[#454545] rounded-lg"
              autoComplete="off"
            />
          </div>
        )}
            ...
            <div className="relative" ref={filterMenuRef}>
              ...
              {isFilterMenuOpen && (
                <div className="absolute top-full mt-2 right-0 md:left-0 w-64 bg-white border border-gray-200 rounded-lg shadow-lg p-4 z-20">
                  <h3 className="font-bold text-[#454545] mb-3 text-sm">Filter by Category</h3>
                    ...
                    {Object.keys(filterOptions).map((category) => (
                      <div key={category}>
                        ...
                        >
                          {category}
                          <FiChevronDown
                            className={`${selectedCategory === category ? "rotate-180" : ""} transform transition-transform`}
                          />
                        </button>
                        {selectedCategory === category && (
                          <div className="flex flex-col gap-2 mt-2 ml-4 border-l border-gray-200 pl-2">
                  ...
                  {(selectedCategory || selectedSubCategory || searchTerm) && (
                    <button
                      ...
                      className="w-full flex items-center justify-center gap-2 mt-4 px-4 py-2 bg-[#C94A4A] text-white rounded-lg hover:bg-[#b03b3b] transition text-sm"
                    >
      ...
      <div className="flex-1 overflow-y-auto border-gray-200 max-h-[80vh]">
        {editingProject ? (
          ...
          <AddProject onCancel={onCancelModify} refreshProjects={fetchProjects} />
        ) : (
          <table className="w-full border-collapse text-left text-sm">
            <thead className="sticky top-0 bg-[#D5CFCC] border-b border-gray-300 z-10">
              <tr>
                <th className="p-3 font-medium text-[#454545]">Order</th>
                <th className="font-medium text-[#454545]">Status</th>
                <th className="p-3 font-medium text-[#454545]">Image</th>
                <th className="p-3 font-medium text-[#454545]">Project Name</th>
                <th className="p-3 font-medium text-[#454545]">Year</th>
                <th className="p-3 font-medium text-[#454545]">Category</th>
                <th className="p-3 font-medium text-[#454545]">Subcategory</th>
                <th className="p-3 font-medium text-[#454545]">Date Created</th>
                <th className="p-3 font-medium text-[#454545]">Last Modified</th>
              </tr>
            </thead>
            <tbody>
              {filteredProjects.map((project, index) => (
                ...
                >
                  ...
                  <td className="p-3 border-b border-gray-200">
                    <span
                      className={`inline-block w-3 h-3 rounded-full ${
                        project.active ? "bg-green-500" : "bg-gray-400"
                      }`}
                    ></span>
                  </td>
                  <td className="p-3 border-b border-gray-200">
                  ...
                  <td className="p-3 border-b border-gray-200">{project.createdAt?.slice(0, 10)}</td>
                  <td className="p-3 border-b border-gray-200">{project.updatedAt?.slice(0, 10)}</td>
                </tr>
              ))}
