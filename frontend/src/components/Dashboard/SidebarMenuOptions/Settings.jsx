...
const COLOR_PRIMARY_HOVER = '#333333'; 
const COLOR_SURFACE_LIGHT = 'bg-[#F5F1EE]';
const COLOR_SURFACE_DEFAULT = 'bg-white';
const COLOR_SURFACE_SUBTLE = 'bg-[#F5F1EE]';
const COLOR_SURFACE_DISABLED = 'bg-[#E0E0E0]';
const COLOR_SURFACE_SECONDARY = '#F08080';
const COLOR_TEXT_TITLE = 'text-[#4A4A4A]';
const COLOR_TEXT_BODY = 'text-[#454545]';
...
const Settings = () => {
    ...
    const [newUserData, setNewUserData] = useState({ name: '', email: '', role: USER_ROLES_DEFINITION[0].role });
    const isSuperAdmin = user.role === 'Super Admin';

    // --- Handlers ---
    const handleNameUpdate = (e) => { e.preventDefault(); alert(`Name updated to ${newName}! (Simulated)`); };
    const handlePasswordChange = (e) => {
        e.preventDefault();
        if (newPassword !== confirmPassword) return alert("Passwords do not match.");
        if (newPassword.length < 8) return alert("Password must be at least 8 chars.");
        alert("Password change initiated! (Simulated)");
        setCurrentPassword(''); setNewPassword(''); setConfirmPassword('');
    };
    const handleRoleChange = (userId, newRole) => setUserList(userList.map(u => u.id === userId ? { ...u, role: newRole } : u));
    const handleUserDelete = (userId) => { if (!window.confirm("Are you sure you want to delete this user?")) return; setUserList(userList.filter(u => u.id !== userId)); };
    const handleAddUserToggle = () => setAddingUser(!addingUser);
    const handleNewUserChange = (field, value) => setNewUserData({ ...newUserData, [field]: value });
    const handleAddUserSubmit = () => {
        if (!newUserData.name || !newUserData.email) return alert("Please fill in all fields.");
        const newUser = { ...newUserData, id: `u${Date.now()}`, status: 'Active' };
        setUserList([...userList, newUser]);
        setNewUserData({ name: '', email: '', role: USER_ROLES_DEFINITION[0].role });
        setAddingUser(false);
        alert("New user added! (Simulated)");
    };
    const handleEnable2FA = () => alert("2FA Setup flow initiated! (Simulated)");
    ...
    const PersonalSettings = () => (
        <div className={`${COLOR_SURFACE_DEFAULT} p-6 rounded-lg shadow-md space-y-8`}>
            <div>
                ...
                <form onSubmit={handleNameUpdate} className="space-y-4">
                    ...
                    <input type="text" value={newName} onChange={(e) => setNewName(e.target.value)}
                        className={`p-3 border border-gray-300 rounded-lg w-full max-w-md focus:ring-1 focus:ring-offset-1 focus:ring-[${COLOR_PRIMARY}] focus:border-[${COLOR_PRIMARY}]`} required />
                    <button type="submit" className={`flex items-center px-4 py-2 text-white rounded-md transition`} style={{ backgroundColor: COLOR_PRIMARY }}>
            ...
            <div>
                ...
                <form onSubmit={handlePasswordChange} className="space-y-4">
                    ...
                    <input type="password" placeholder="New Password" value={newPassword} onChange={(e) => setNewPassword(e.target.value)}
                        className={`p-3 border border-gray-300 rounded-lg w-full max-w-md focus:ring-1 focus:ring-offset-1 focus:ring-[${COLOR_PRIMARY}] focus:border-[${COLOR_PRIMARY}]`} required minLength={8} />
            ...
            <div>
                ...
                <p className={`text-sm ${COLOR_TEXT_BODY} mb-4`}>Enable 2FA for extra security.</p>
                <button className={`flex items-center px-4 py-2 text-white rounded-md transition text-sm`} style={{ backgroundColor: '#4A4A4A' }} onClick={handleEnable2FA}>
        ...
        <div className={`${COLOR_SURFACE_DEFAULT} p-6 rounded-lg shadow-md mt-8 space-y-10`}>
            ...
            <div className="overflow-x-auto">
                <div className="flex justify-end mb-4">
                    <button className={`flex items-center px-4 py-2 text-white rounded-md transition text-sm`} style={{ backgroundColor: COLOR_PRIMARY }} onClick={handleAddUserToggle}>
                ...
                {addingUser && (
                    <div className={`${COLOR_SURFACE_SUBTLE} p-4 mb-4 rounded-lg border border-gray-300 space-y-2`}>
                        <input type="text" placeholder="Name" value={newUserData.name} onChange={(e) => handleNewUserChange('name', e.target.value)}
                            className="p-2 border border-gray-300 rounded w-full max-w-md" />
                        <input type="email" placeholder="Email" value={newUserData.email} onChange={(e) => handleNewUserChange('email', e.target.value)}
                            className="p-2 border border-gray-300 rounded w-full max-w-md" />
                        <select value={newUserData.role} onChange={(e) => handleNewUserChange('role', e.target.value)}
                            className="p-2 border border-gray-300 rounded w-full max-w-md">
                            {USER_ROLES_DEFINITION.map(role => <option key={role.access} value={role.role}>{role.role}</option>)}
                        </select>
                        <button onClick={handleAddUserSubmit} className={`px-4 py-2 text-white rounded-md transition`} style={{ backgroundColor: COLOR_PRIMARY }}>
                            Add User
                        </button>
                    </div>
                )}
                ...
                <table className="min-w-full divide-y divide-gray-200 border border-gray-200">
                    ...
                    <tbody className={`${COLOR_SURFACE_DEFAULT} divide-y divide-gray-200`}>
                        {userList.map(u => (
                            <tr key={u.id} className={u.id === user.id ? 'bg-gray-50' : ''}>
                                <td className="px-6 py-4 whitespace-nowrap text-sm font-medium">{u.name}</td>
                                <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-600">{u.email}</td>
                                <td className="px-6 py-4 whitespace-nowrap text-sm">
                                    <select value={u.role} onChange={(e) => handleRoleChange(u.id, e.target.value)}
                                        className={`border p-1 rounded-md text-sm bg-white ${COLOR_TEXT_BODY}`} disabled={u.id === user.id}>
                                        {USER_ROLES_DEFINITION.map(role => (
                                            <option key={role.access} value={role.role}>{role.role} (Level {role.access})</option>
                                        ))}
                                    </select>
                                </td>
                                <td className="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                                    {u.id !== user.id ? <button onClick={() => handleUserDelete(u.id)} className="text-red-600 hover:text-red-800 ml-4 p-1"><FiTrash2 /></button> : <span className="text-gray-500 text-xs italic">Current User</span>}
                                </td>
                            </tr>
                        ))}
                    </tbody>
            ...
            {/* Remaining sections (Access Definitions & Activity Log) unchanged */}
            <div className='mt-10'>
                ...
                <div className="text-sm text-gray-700 mb-4 flex items-center p-2 bg-yellow-100 border border-yellow-300 rounded">
                    <FiAlertCircle className="mr-2 text-yellow-700" /> Reference table for access levels
                </div>
                <div className="overflow-x-auto">
                    <table className="min-w-full divide-y divide-gray-200 border">
                        <thead style={{ backgroundColor: '#F1E4DF' }} className={`${COLOR_TEXT_PRIMARY}`}>
                            <tr>{['Role', 'Access Level', 'Who?', 'Responsibilities'].map(h => <th key={h} className="px-4 py-2 text-left text-xs font-bold uppercase tracking-wider">{h}</th>)}</tr>
                        </thead>
                        <tbody className={`${COLOR_SURFACE_DEFAULT} divide-y divide-gray-200`}>
            ...
            <div className='mt-10'>
                {renderHeader(FiShield, "Recent Activity Log")}
