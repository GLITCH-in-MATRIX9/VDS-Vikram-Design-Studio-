      ...
      <nav className="w-full bg-[#f3efee] border-b border-[#e0dcd7] relative z-50">
        <div className="max-w-7xl mx-auto px-6 py-4 flex justify-between items-center">
          ...
          <div className="flex items-center gap-4">
            {/* Conditional visibility based on the page */}
            <div className={`flex items-center gap-4 hidden md:flex ${!isHome && 'invisible pointer-events-none'}`}>
              {/* Filter button */}
              <button
                onClick={() => {
              ...
              {Object.keys(filterOptions).map((category) => (
                <button
                  ...
                  className={`px-4 py-2 rounded-md font-medium transition ${
                    selectedCategory === category ? "bg-gray-700 text-white" : "text-gray-700 hover:bg-gray-200"
                  } text-xs sm:text-sm`}
                >
                  {category}
                </button>
