...
const Navbar = () => {
  ...
  const { categoryContext, setCategoryContext, subCategoryContext, setSubCategoryContext, searchQuery, setSearchQuery, clearFilters } = useFilters();
    ...
    <>
        ...
        <div className="mx-auto py-4 flex justify-between items-center">
          ...
          <div className="flex items-center gap-4">
            ...
            <div className={`hidden md:flex items-center gap-4 uppercase text-[#474545] ${!isHome && "invisible pointer-events-none"}`}>
              {!filterOpen ? (
                <>
                  <button onClick={() => { setFilterOpen(true); setMenuOpen(false); }}>
            ...
            <div className="max-w-7xl mx-auto px-6 py-3 flex flex-wrap justify-evenly gap-2">
              {Object.keys(filterOptions).map((category) => (
                <button
                  ...
                  className={`flex gap-2 items-center px-4 py-2 rounded-md font-medium uppercase ${selectedCategory === category ? "text-[#474545]" : "text-[#BEBBBC]"} text-xs xl:text-sm`}
                >
                  <img src={CategorySelectedIcon} className={`w-3 h-3 opacity-0 ${selectedCategory === category && "opacity-100"}`} />
                  {category}
                </button>
              ))}
            </div>
      ...
      <AnimatePresence>
        {isHome && selectedCategory && filterOpen && (
          <motion.div key="subfilters" initial={{ height: 0, opacity: 0 }} animate={{ height: "auto", opacity: 1 }} exit={{ height: 0, opacity: 0 }} className="w-full bg-[#F2EFEE] overflow-hidden z-30">
            <div className="max-w-7xl mx-auto px-6 py-3 flex justify-evenly flex-wrap gap-2 text-[#474545]">
              {filterOptions[selectedCategory].map((sub, i) => (
                <div
                  ...
                  className={`px-3 py-1 text-[10px] rounded-md cursor-pointer uppercase xl:text-xs flex gap-1 items-center ${selectedSubCategory === sub ? "text-[#474545]" : "text-[#BEBBBC]"}`}
                >
                  <img src={SubCategorySelectedIcon} className={`w-[5.25px] h-[5.25px] opacity-0 ${selectedSubCategory === sub && "opacity-100"}`} />
                  {sub}
                </div>
              ))}
            </div>
      ...
      <AnimatePresence>
        {menuOpen && (
          <motion.div key="mobile-menu" initial={{ x: "100%" }} animate={{ x: "0%" }} exit={{ x: "100%" }} transition={{ duration: 1 }} className="fixed top-0 right-0 z-[100] bg-[#f9f6f3] w-[187px] h-full shadow-lg">
            <div className="w-full h-full flex flex-col">
              <div className="w-full flex justify-end pt-6 pr-6">
                <button onClick={closeAllMenus}><img src={closeIcon} alt="close" className="h-4" /></button>
              </div>
              ...
              <div className="mt-auto flex flex-col gap-8 px-4 pb-6">
                ...
                <p className="uppercase text-[8px] text-[#5D5D5D]">Vikram Design Studio @ 2025. All rights reserved.</p>
              </div>
            </div>
          </motion.div>
        )}
