...
const HorizontalScrollComponent = ({ onClose, project }) => {
  ...
  return (
    <div className="relative w-full font-inter overflow-hidden">
      ...
      <button
        onClick={scrollLeft}
        className="absolute z-10 left-2 top-1/2 transform -translate-y-1/2 bg-white/80 hover:bg-white text-gray-500 p-2 rounded-full shadow-md transition hidden lg:flex"
      >
        <FiArrowLeft size={16} />
      </button>
      ...
      <button
        onClick={scrollRight}
        className="absolute z-10 right-2 top-1/2 transform -translate-y-1/2 bg-white/80 hover:bg-white text-gray-500 p-2 rounded-full shadow-md transition hidden lg:flex"
      >
